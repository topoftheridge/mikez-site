    ...
    <footer className="bg-dark-light border-t border-white/10 text-gray-400">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div>
            <div className="mb-4">
              <Image src="/logoheader.png" alt="Mikez Mobile Detailing" width={170} height={100} className="h-16 w-auto" />
            </div>
            ...
            <div className="mt-4">
              <a href="https://www.instagram.com/mikezmobiledetailing/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-gray-400 hover:text-primary transition">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {locations.map((loc) => (
                <li key={loc.slug}>
                  <Link href={`/${loc.slug}`} className="hover:text-primary transition">{loc.name}, OH</Link>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 text-center text-sm text-gray-500">
          <p>© {new Date().getFullYear()} Mikez Mobile Detailing LLC. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
